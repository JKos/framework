...
function setHook(hookIdString,func,ordinal)
	...
	if fw.hooks[hookNamespace][hooksId][hookFuncName] then
		print('Warning: Hook '..hookIdString..' was owerwritten')
	else
...
for i=1,# lists.monster do 
	cloneObject{
		name = lists.monster[i],
		baseObject = lists.monster[i],
...
for i=1,# lists.door do 
	cloneObject{
		name = lists.door[i],
		baseObject = lists.door[i],
...
for i=1,# lists.blockage do 
	cloneObject{
		name = lists.blockage[i],
		baseObject = lists.blockage[i],
...
function fw_cloneObject(objDef)
	cloneObject(objDef)
	-- put item in consumbles list if consumable = true or baseObject is consumable and consumable in not defined
	if objDef.consumable or (objDef.consumable == nil and lists.item.consumable[objDef.baseObject]) then
		lists.item.consumable[objDef.name] = true
	else
		--lists.item[objDef.][]
	end
end
...
fw_addModuleInitCallback('fw',function(fw)
